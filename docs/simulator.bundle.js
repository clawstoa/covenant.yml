 ...
 * Built: 2026-02-21T16:18:06.645Z
 */
...
(function attachSimulator(global) {
  ...
  function buildStoryChoices(input) {
    ...
    choices.push({
      id: "reroute_branch",
      label: "Retarget to develop-agent",
      explanation: "Retargets to develop-agent to probe branch-sensitive rules.",
    });
  ...
  function applyStoryChoice(input) {
    ...
    if (input.choice_id === "add_provenance") {
      ...
      event.target = Object.assign({}, event.target, { branch: "develop-agent" });
    }
    return event;
  }
  ...
  function simulateRun(input) {
    ...
    for (var i = 0; i < count; i += 1) {
      ...
      var actorId = actorKind === "agent" ? "ci-agent[bot]" : (actorKind === "manager" ? "stoa-manager[bot]" : "alice");
      ...
      var event = {
        ...
        target: {
          branch: rng.pick(["main", "develop", "develop-agent", "release"]) || "main",
          labels: labels,
